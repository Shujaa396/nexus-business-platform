      ...
      <div className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        ...
        {data && !dashboardQuery.isLoading && !dashboardQuery.isError && (
          <>
            ...
            <section className="rounded-xl border border-border bg-white p-5 shadow-sm"><h2 className="text-base font-semibold text-slate-950">Warehouse Valuation</h2><p className="mt-1 text-xs text-slate-500">Physical stock value by active warehouse.</p><AnalyticsTable headers={["Warehouse", "Products", "Low stock", "Value"]} rows={data.warehouses.data.warehouses.map((item) => [item.name, formatNumber(item.product_count), formatNumber(item.low_stock_count), formatCurrency(item.inventory_value)])} /></section>
          </>
        )}
      </div>
    </AppShell>
